plot.surv <-
  function(x = object, y = NULL, conf = NULL,  type = NULL, conftype = NULL,
    ...
    if(object$Nlevels != length(col)) col <- rep(col, times = object$Nlevels)
    if(object$Nlevels !=  length(confcol)) confcol <- rep(confcol, times = object$Nlevels)
    ...
    if (object$Nlevels == 1){
      plot(ob[, 1], ob[, 2], type = type, col = col, xlab = xlab, ylab = ylab,
           ...)
      if (ci == TRUE){
      ...
      plot(ob[[1]][, 1], ob[[1]][, 2], type = "n", xlab = xlab, ylab = ylab,
           ...)

      for(i in 1:object$Nlevels){
        lines(ob[[i]][, 1], ob[[i]][, 2], type = type, col = col[i], lty = lty, ...)
        if (ci == TRUE){
          lines(x = ob[[i]][, 1], y = ob[[i]][, 3], type = conftype, lty = conflty,
                col = confcol[i], ...)
          lines(x = ob[[i]][, 1], y = ob[[i]][, 4], type = conftype, lty = conflty,
                col = confcol[i], ...)
