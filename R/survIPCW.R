survIPCW <-
function(object, x, y, z.name, z.value, bw = "dpik", window = "gaussian", method.weights = "NW",
conf = FALSE, n.boot = 200, conf.level = 0.95, lower.tail = FALSE)
{
    if (missing(object))
...
ifelse (any(be[p1] == 0) | any(be2[p2] == 0), res[j] <- NA, res[j] <- 1 - sum(w1[p1] / be[p1]) / (sum(w1[p2] / be2[p2])))
    ...
    if (conf == TRUE)
           {
           res.ci <- matrix(0, nrow = n.auxi, ncol = n.boot)
           for (k in 1:n.boot)
                   {
   ...
   ifelse (is.numeric(bw), lbd2 <- bw, lbd2 <- dpik(x = ndata[, covar]))
                   ...
                   if (auxi == 1 | auxi == 3) {
                            for (j in 1:n.auxi) {       
                                    ...
                                    ifelse(method.weights == "NW", w1 <- NWW(ndata[, covar], z.value[j], kernel = window, bw = lbd2), w1 <- LLW(ndata[, covar], bw = lbd2, t1 = z.value[j]))
    if (lower.tail == FALSE) {
